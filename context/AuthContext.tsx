...
export const AuthProvider = ({ children }: any) => {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      const result = await fetch(`https://streamlivechat.onrender.com/login`, {
        method: "POST",
        headers: {
      ...
      await AsyncStorage.setItem(TOKEN_KEY, JSON.stringify(json));
  ...
  const register = async (email: string, password: string) => {
    try {
      const result = await fetch(
        `https://streamlivechat.onrender.com/register`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
